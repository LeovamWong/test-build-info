...
LDFlags=" \
    -X 'test-build-info/bininfo.GitCommitLog=${GitCommitLog}' \
    -X 'test-build-info/bininfo.GitStatus=${GitStatus}' \
    -X 'test-build-info/bininfo.BuildTime=${BuildTime}' \
    -X 'test-build-info/bininfo.BuildGoVersion=${BuildGoVersion}' \
"
...
# 编译多个可执行程序
#cd ${ROOT_DIR}/.. && go build -ldflags "$LDFlags" -o ${ROOT_DIR}/bin/add_blog_license &&
...
cd ${ROOT_DIR}/.. && go build -ldflags "$LDFlags" -o ${ROOT_DIR}/bin/myapp &&
ls -lrt ${ROOT_DIR}/bin &&
